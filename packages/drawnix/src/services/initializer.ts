...
import { memoryMonitorService, MEMORY_CRITICAL_EVENT } from './memory-monitor-service';
import { unifiedCacheService } from './unified-cache-service';
import { thumbnailCache } from '../utils/thumbnail-generator';
...
export class DrawnixServiceInitializer {
  ...
  private initMemoryMonitor(): void {
    try {
      // 订阅内存熔断事件
      this.handleMemoryCritical = this.handleMemoryCritical.bind(this);
      window.addEventListener(MEMORY_CRITICAL_EVENT, this.handleMemoryCritical);
      console.log('[Drawnix] Memory critical event listener registered');

      // 启动内存监控
      memoryMonitorService.startMonitoring();
      console.log('[Drawnix] Memory monitoring started');
    } catch (error) {
      console.warn('[Drawnix] Failed to start memory monitor:', error);
    }
  }

  /**
   * 处理内存熔断事件
   * 当内存使用率超过阈值时，清空所有缓存释放内存
   */
  private handleMemoryCritical(): void {
    console.warn('[Drawnix] Handling memory critical event - clearing caches');

    // 清空所有 Blob 缓存（释放内存）
    unifiedCacheService.clearAllBlobs();

    // 清空预注入缓存
    unifiedCacheService.clearMemoryInjectionCache?.();

    // 清空待处理的请求
    unifiedCacheService.clearPendingRequests?.();
    ...
    if (window.gc) {
      window.gc();
      console.log('[Drawnix] Manual GC triggered');
    }

    console.log('[Drawnix] Cache cleared - memory should be released');
  }
  ...
  async cleanup(): Promise<void> {
    ...
    window.removeEventListener(MEMORY_CRITICAL_EVENT, this.handleMemoryCritical);
