...
class StorageMonitorService {
  ...
  async requestPersistentStorage(): Promise<boolean> {
    if (typeof navigator === 'undefined' || !navigator.storage?.persist) {
      console.log('[StorageMonitor] Browser does not support persistent storage');
      return false;
    }
    ...
    try {
      ...
      if (isPersisted) {
        ...
        console.log('[StorageMonitor] Persistent storage not granted (browser limited storage)');
      }

      return isPersisted;
    } catch (e) {
      console.log('[StorageMonitor] Failed to request persistent storage:', e);
      return false;
    }
  }
