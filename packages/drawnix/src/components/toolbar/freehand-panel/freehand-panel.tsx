...
import { DrawnixPointerTypeValue } from '../../../hooks/use-drawnix';
...
export type FreehandPickerProps = {
  onPointerUp: (pointer: DrawnixPointerTypeValue) => void;
};
              ...
              {rowFreehands.map((freehand, index) => {
                return (
                  <ToolButton
                    ...
                    visible={true}
                    icon={freehand.icon}
                    title={t(freehand.titleKey as any)}
                    aria-label={t(freehand.titleKey as any)}
                    onPointerDown={() => {
