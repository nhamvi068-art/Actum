...
import './task-notification-panel.scss';
...
export interface TaskNotificationPanelProps {
  ...
  onTaskRedo?: (task: GenerationTask) => void;
  onTaskClick?: (task: GenerationTask) => void;
  onTaskCancel?: (task: GenerationTask) => void;
}
...
export const TaskNotificationPanel: React.FC<TaskNotificationPanelProps> = ({
  ...
  useEffect(() => {
    const loadTasks = () => {
      const allTasks = getAllTasksMemory();
      const projectTasks = allTasks.filter(t => t.workspaceId === projectId || t.projectId === projectId);
      projectTasks.sort((a, b) => b.createdAt - a.createdAt);
      setTasks(projectTasks);
    };

    loadTasks();

    const unsubAdded = onTaskAdded(loadTasks);
    const unsubUpdated = onTaskUpdated(loadTasks);
    const unsubRemoved = onTaskRemoved(loadTasks);
  ...
  const handleRetry = (task: GenerationTask, e: React.MouseEvent) => {
    e.stopPropagation();
    if (onTaskRetry) {
      onTaskRetry(task);
    }
  ...
  const handleRedo = (task: GenerationTask, e: React.MouseEvent) => {
    e.stopPropagation();
    if (onTaskRedo) {
      onTaskRedo(task);
    }
  ...
  const handleCancel = (task: GenerationTask, e: React.MouseEvent) => {
    e.stopPropagation();
    updateTaskStatusMemory(task.id, 'failure');
    if (onTaskCancel) {
      onTaskCancel(task);
    }
    const allTasks = getAllTasksMemory();
