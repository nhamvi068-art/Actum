 ...
 * 创建 Freehand 元素
 *
 * @param board - PlaitBoard 实例，用于读取当前预设配置
 * @param shape - 画笔形状类型
 * @param points - 线条坐标点数组
 * @returns 完整的 Freehand 元素，包含预设的 strokeColor, strokeWidth, opacity 等属性
 */
...
export const getSelectedFreehandElements = (board: PlaitBoard) => {
  return board.getSelectedElements().filter((ele) => Freehand.isFreehand(ele));
};
