  ...
  if (selectedElements.length > 0) {
    selectedElements.forEach(element => {
      ...
      editors.forEach(editor => {
        if (isNoColor(newColor)) {
          // Remove color mark - use Editor.removeMark from slate
          Editor.removeMark(editor, 'color');
        } else {
          const colorWithOpacity = applyOpacityToHex(newColor, currentOpacity);
          PlaitMarkEditor.setColorMark(editor, colorWithOpacity);
        }
      });
    });
  }
...
export const setTextColorOpacity = (
  ...
  if (selectedElements.length > 0) {
    selectedElements.forEach(element => {
      const editors = getTextEditorsByElement(element);
      editors.forEach(editor => {
        PlaitMarkEditor.setColorMark(editor, newFontColor);
      });
    });
  }
};
