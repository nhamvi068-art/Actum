...
import { useTaskQueue, useTaskActions, removeTaskMemory } from '@drawnix/drawnix';
import { assetStorageService } from '@drawnix/drawnix';
import {
  History,
  RefreshCw,
  Trash2,
...
export interface TaskListButtonProps {
  ...
  onSendToCanvas?: (task: ImageTask) => void;
}
...
export const TaskListButton: React.FC<TaskListButtonProps> = ({
  ...
  const tasks = useMemo(() => {
    const projectTasks = allTasks
      ...
      .map(t => ({
        ...
        // 优先使用任务根字段 resultImageUrl（应用层写入），兜底 result.remoteUrl（兼容旧数据）
        resultImageUrl: (t as any).resultImageUrl ?? t.result?.remoteUrl,
        originalUrl: (t as any).originalUrl,
        localAssetId: (t as any).localAssetId,
      }));
  ...
  const handleDelete = async (taskId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    // 同时清理 IndexedDB 和内存缓存，防止刷新后任务"复活"
  ...
  const handleDownload = async (task: ImageTask, e: React.MouseEvent) => {
    e.stopPropagation();
    const src = task.resultImageUrl || task.originalUrl;
    if (!src) {
      alert('图片尚未生成或生成失败，无法下载。');
      return;
    }

    try {
      // 优先从 IndexedDB 本地 blob 下载（完整原图，无需远程认证）
      if (task.localAssetId) {
        const asset = await assetStorageService.getAsset(task.localAssetId);
        if (asset?.blob) {
          const mimeType = asset.mimeType || 'image/png';
          const url = URL.createObjectURL(asset.blob);
          const a = document.createElement('a');
          a.href = url;
          a.download = `generated-image-${task.id}.${mimeType.split('/')[1] || 'png'}`;
      ...
      if (src.startsWith('data:')) {
        ...
        const blob = new Blob([bytes], { type: src.match(/data:([^;]+)/)?.[1] || 'image/png' });
  ...
  // 处理发送到画布
  const handleSendToCanvas = (task: ImageTask, e: React.MouseEvent) => {
    e.stopPropagation();
    const imageUrl = task.resultImageUrl || task.originalUrl;
    if (!imageUrl) {
      alert('图片生成失败，无法发送到画布。请尝试重新生成。');
      return;
    }
    if (onSendToCanvas) {
      onSendToCanvas(task);
  ...
  // 处理复制提示词
  const handleCopyPrompt = async (task: ImageTask, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleClearAll = () => {
    const confirmed = window.confirm(`确定要清空全部 ${tasks.length} 条生成记录吗？此操作不可撤销。`);
    if (!confirmed) return;
    tasks.forEach(task => {
                          ...
                          <div className="history-item-actions">
                            ...
                            {(displayStatus === 'success' || displayStatus === 'failed') && (
                              <button
                                ...
                                title={displayStatus === 'success' ? '重新生成' : '重试'}
                              >
                            ...
                            {displayStatus === 'success' && (
                              <button
                                onClick={(e) => handleSendToCanvas(task, e)}
                                className="history-item-action-btn history-item-action-btn--primary"
                                title="发送到画布"
                              >
                                <ImageIcon className="history-item-action-icon" />
                              </button>
                            )}
                            <button
